      ...
      if (isset($_GET['id'])) {
        ...
        if ($idPage!=''&&$idPage!=0) {
          ...
          if (isset($_SESSION['idsessionedition'])) {
            $_SESSION["idsessionedition"] .= (string)$idPage.';';
          } else{
            $_SESSION["idsessionedition"] = ';'.(string)$idPage.';';
          }
  ...
  <body style="background-color:#D8D8D8;" >
    ...
    <div id="gjs" style="height:0px; overflow:hidden" >
      <?php
        echo($base_html);
        if($base_css!=''){
          $base_css = str_replace("height:0px;","",$base_css);
          echo('<style>'.$base_css.'</style>');
        }
        $plugcss = file_get_contents("templates/styles/plug.css");
        echo('<style>');
        echo($plugcss);
        echo('
          .cell{
    ...
    <?php
      ...
      if(file_exists($sysWinston)){
        echo "<script>var _p = {web_plugin: '".api_get_path(WEB_PLUGIN_PATH)."',web_editor: '".api_get_path(WEB_PLUGIN_PATH)."adv_oel_tools_teachdoc/editor'};</script>";
