...
{% for term in terms %}
    <tr>
        ...
        <td style="width:20%" >
			<a target="_blank" href="{{ _p.web_plugin }}adv_oel_tools_teachdoc/teachdoc-render.php?{{ {'id': term.id}|url_encode() }}" >
            <img src="img/eyes.png" /></a>
        </td>
        <td style="width:15%" >

			<a href="{{ _p.web_plugin }}adv_oel_tools_teachdoc/editor/index.php?action=edit&{{ {'id': term.id}|url_encode() }}" 
			class="btn btn-success">
            <span class="fa fa-edit fa-fw" aria-hidden="true"></span></a>    
			<a onclick="return confirm('Etes-vous sure ?')" 
			href="{{ _p.web_plugin }}adv_oel_tools_teachdoc/oel_tools_teachdoc_list.php?action=delete&{{ {'id': term.id}|url_encode() }}" 
